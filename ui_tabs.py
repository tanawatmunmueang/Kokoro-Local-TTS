...
from video_logic import generate_video_from_media, generate_video_from_sequence
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                shuffle_media_checkbox = gr.Checkbox(
                    label="Shuffle & Sequence Media",
                    value=True,
                    info="Creates a looping visual sequence from all media. Bypasses count check. FPS defaults to 30.",
                    interactive=True,
                    visible=False
                )
                ...
                video_frame_rate_slider = gr.Slider(
                    label="Video Frame Rate (FPS)", minimum=1, maximum=60, value=30, step=1
                )
        ...
        def update_pairings_display(bulk_mode, shuffle_enabled, audio_paths, cover_paths):
            ...
            if shuffle_enabled:
                ...
                pairing_text = (
                    f"### Shuffle & Sequence Mode Enabled\n\n"
                    f"**{num_audio}** audio file(s) and **{num_cover}** media file(s) are ready.\n"
                    f"For each audio, a video will be generated by creating a looping sequence from the re-shuffled media."
                )
            ...
            if num_cover == 1:
                ...
                pairing_text = (
                    ...
                    f"Please provide either **1 cover file** for all audio, **one cover file for each audio file**, or enable Shuffle & Sequence Mode."
                )
            return gr.update(visible=True), pairing_text

        def toggle_bulk_mode(bulk_enabled, audio_files, cover_files):
            if bulk_enabled:
                return gr.update(file_count="multiple", value=[]), gr.update(file_count="multiple", value=[]), gr.update(visible=True)
            else: # Single mode
                ...
                return gr.update(file_count="single", value=new_audio), gr.update(file_count="single", value=new_cover), gr.update(visible=False)

        def update_file_inputs(audio_files, cover_files, bulk_mode, shuffle_enabled):
            ...
            can_generate = False
            if audio_paths and cover_paths:
                if not bulk_mode:
                    can_generate = True
                else: # Bulk mode is on
                    if shuffle_enabled:
                        can_generate = True # Shuffle mode doesn't need matching counts
                    else: # Bulk mode without shuffle requires matching counts
                        if len(cover_paths) == 1 or len(cover_paths) == len(audio_paths):
                            can_generate = True
        ...
        def start_generation(bulk_mode, shuffle_enabled, audio_paths, cover_paths, res, enc, fps_slider_val, save_dir, progress=gr.Progress()):
            log_text = ""
            fps = 30 if (bulk_mode and shuffle_enabled) else fps_slider_val

            generated_files = []
            final_info_text = ""

            if bulk_mode and shuffle_enabled:
                if not audio_paths or not cover_paths:
                    msg = "Error: Audio and Cover files must be provided for this mode."
                    gr.Error(msg)
                    # Yield an error message to the log
                    yield None, None, msg
                    return

                for i, audio_p in enumerate(audio_paths):
                    status_message = f"Processing audio {i + 1} of {len(audio_paths)}: {os.path.basename(audio_p)}..."
                    log_text += status_message + "\n"
                    progress(i / len(audio_paths), desc=status_message)
                    yield None, None, log_text

                    # Re-shuffle the media list for EACH audio file
                    mutable_cover_paths = list(cover_paths)
                    random.shuffle(mutable_cover_paths)

                    # Call the new sequence generator function from video_logic
                    video_path, info_text = generate_video_from_sequence(audio_p, mutable_cover_paths, res, enc, fps)

                    if video_path:
                        generated_files.append(video_path)
                        final_info_text = info_text
                        log_text += f"-> Generated Sequence Video: {os.path.basename(video_path)}\n"
                        ...
                        log_text += f"-> FAILED to generate sequence video. Check logs.\n"
                    yield None, None, log_text

            else: # Original logic for single file or standard bulk mode
                pairs = []
                if not bulk_mode:
        ...
        def toggle_fps_slider(bulk_mode, shuffle_enabled):
            if bulk_mode and shuffle_enabled:
                return gr.update(interactive=False, value=30)
            else:
                return gr.update(interactive=True)

        # --- Event Wiring ---
        pairings_inputs = [bulk_mode_checkbox, shuffle_media_checkbox, audio_filepaths_state, cover_filepaths_state]
        pairings_outputs = [pairings_accordion, file_pairings_display]
        file_inputs_and_flags = [audio_input, cover_input, bulk_mode_checkbox, shuffle_media_checkbox]
        file_input_outputs = [generate_video_btn, audio_filepaths_state, cover_filepaths_state]

        bulk_mode_checkbox.change(
            fn=toggle_bulk_mode,
            inputs=[bulk_mode_checkbox, audio_filepaths_state, cover_filepaths_state],
            outputs=[audio_input, cover_input, shuffle_media_checkbox]
        ).then(
            fn=update_pairings_display,
            inputs=pairings_inputs,
            outputs=pairings_outputs
        ).then(
            fn=toggle_fps_slider,
            inputs=[bulk_mode_checkbox, shuffle_media_checkbox],
            outputs=[video_frame_rate_slider]
        )

        shuffle_media_checkbox.change(
            fn=toggle_fps_slider,
            inputs=[bulk_mode_checkbox, shuffle_media_checkbox],
            outputs=[video_frame_rate_slider]
        ).then(
            ...
            inputs=file_inputs_and_flags,
            outputs=file_input_outputs
        )
        ...
        generate_video_btn.click(
            ...
            fn=start_generation,
            inputs=[bulk_mode_checkbox, shuffle_media_checkbox, audio_filepaths_state, cover_filepaths_state, resolution_select, encoder_select, video_frame_rate_slider, local_save_path_input],
